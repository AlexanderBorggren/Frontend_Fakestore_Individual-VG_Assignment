window.addEventListener ('load', function () {
  ...
  allProducts.forEach (selectedProduct => {
    ...
    function updateCart () {
      ...
      totalPriceSum = allProducts.reduce (
        (sum, product) => sum + product.price * product.quantity,
        0
      );
      totalProductsCount = allProducts.reduce (
  ...
  let totalProductsCountElement = document.querySelector (
    '#totalProductsCount'
  );
  let totalProductsCountText = document.createTextNode (
